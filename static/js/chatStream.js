...
export function handleUIControl(uiData) {
  ...
  try {
    if (uiEvent === 'toggle' || uiData.ui_event === 'toggle') {
      ...
      document.querySelectorAll('.aigenchain-highlight').forEach(function(e) { e.classList.remove('aigenchain-highlight'); });
      document.querySelectorAll('.aigenchain-hl-label').forEach(function(e) { e.remove(); });
      var target = document.querySelector(uiData.selector);
      if (target) {
        target.classList.add('aigenchain-highlight');
        target.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
        if (uiData.label) {
          var lbl = document.createElement('div');
          lbl.className = 'aigenchain-hl-label';
