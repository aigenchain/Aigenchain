...
function _renderHwVisibilityWarning(sys) {
  ...
  box.querySelector('[data-hw-action="copy"]')?.addEventListener('click', () => {
    ...
    const text = [
      'Aigenchain Cookbook hardware diagnostics',
      ...
      '',
      'Useful checks:',
      'docker compose exec aigenchain nvidia-smi -L',
      'docker compose exec aigenchain cat /proc/meminfo | head',
      'docker compose exec aigenchain python -c "from services.hwfit.hardware import detect_system; import json; print(json.dumps(detect_system(fresh=True), indent=2))"',
    ].join('\n');
...
export function _hwfitInit() {
  ...
  function _syncServers() {
    ...
    // dropdown still showed aigenchain. The user's selection must only change via
