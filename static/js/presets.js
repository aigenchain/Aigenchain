  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (isBuiltin) {
          const hidden = loadStoredArray('aigenchain-hidden-presets');
          if (!hidden.includes(charName)) hidden.push(charName);
          localStorage.setItem('aigenchain-hidden-presets', JSON.stringify(hidden));
        }
...
function _populateCharSelect() {
  ...
  const hiddenPresets = loadStoredArray('aigenchain-hidden-presets');
...
function initPersistentChat() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const charSessions = loadStoredObject('aigenchain-char-sessions');
      charSessions[sessionId] = charName;
      localStorage.setItem('aigenchain-char-sessions', JSON.stringify(charSessions));
