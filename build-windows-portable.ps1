...
<#
  Build a portable Windows distribution for Aigenchain.

  Output layout:
    dist\Aigenchain\Aigenchain.exe
    dist\Aigenchain\static\...
    dist\Aigenchain\scripts\...
    dist\Aigenchain\mcp_servers\...
    dist\Aigenchain\services\hwfit\data\...
...
& $pyExe -m PyInstaller --noconfirm --clean --onedir --noconsole --icon=static/icon.ico --name Aigenchain @dataArgs launcher.py
if ($LASTEXITCODE -ne 0) { Fail "PyInstaller build failed." }

Write-Host ""
Write-Host "Build complete." -ForegroundColor Green
Write-Host "Portable app folder: $PSScriptRoot\dist\Aigenchain" -ForegroundColor Green
