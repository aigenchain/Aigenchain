"""Regression: `aigenchain-calendar list` must select events that OVERLAP the
...
`aigenchain-calendar list` with the default start=now()).
"""
...
def _load_cli(monkeypatch, rows):
    ...
    path = ROOT / "scripts" / "aigenchain-calendar"
    loader = importlib.machinery.SourceFileLoader("aigenchain_calendar_cli", str(path))
