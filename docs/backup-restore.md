...
Aigenchain keeps all of your state in the `data/` directory — the SQLite database
(`app.db`), the Fernet encryption key (`data/.app_key`), the vault, memory, RAG
indexes, personal documents, and uploads. The `scripts/aigenchain-backup` tool
...
```bash
# Create a snapshot → backups/aigenchain-backup-<YYYYMMDD-HHMMSS>.tar.gz
./scripts/aigenchain-backup snapshot

# List existing snapshots (most recent first)
./scripts/aigenchain-backup list

# Check a tarball's integrity without extracting it
./scripts/aigenchain-backup verify backups/aigenchain-backup-20260101-120000.tar.gz

# Restore (destructive — see the warning below)
./scripts/aigenchain-backup restore backups/aigenchain-backup-20260101-120000.tar.gz --yes
```
...
./scripts/aigenchain-backup snapshot --out /mnt/nas/aigenchain-$(date +%F).tar.gz

# Full snapshot including research runs and mail attachments
./scripts/aigenchain-backup snapshot --include-research --include-attachments
```
...
```cron
0 3 * * *  cd /path/to/aigenchain && ./scripts/aigenchain-backup snapshot --out "/mnt/nas/aigenchain-$(date +\%F).tar.gz"
```
...
> **not** under `./data`. `aigenchain-backup` therefore does not capture the Docker
