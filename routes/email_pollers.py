                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Aigenchain (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Aigenchain" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "aigenchain_kind" if "aigenchain_kind" in cols else "'scheduled' AS aigenchain_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                aigenchain_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Aigenchain-Origin"] = "aigenchain-ui"
                outer["X-Aigenchain-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", aigenchain_kind or "scheduled")[:64]
                outer["X-Aigenchain-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `aigenchain-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `AIGENCHAIN_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `aigenchain-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("AIGENCHAIN_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `AIGENCHAIN_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (AIGENCHAIN_INPROCESS_POLLERS=0); "
            "drive `aigenchain-mail poll-scheduled` externally."
        )
        return
    import asyncio
