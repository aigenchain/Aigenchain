...
DATA_DIR = os.getenv("AIGENCHAIN_DATA_DIR", get_default_data_dir())

# Data file paths
# Single source of truth: every persisted file/dir lives under DATA_DIR, which
# is the ONLY place AIGENCHAIN_DATA_DIR is read. Import these constants instead of
...
IMAGE_PROVIDERS_FILE = os.path.join(DATA_DIR, "image_providers.json")
...
# Piper TTS (offline, CPU) — needs the `piper-tts` Python package installed
# (`pip install piper-tts`) and voice models (<voice>.onnx + <voice>.onnx.json)
# dropped into PIPER_VOICES_DIR. The tts_voice setting is the model filename
# stem, e.g. "id_ID-news_tts-medium".
PIPER_VOICES_DIR = os.path.join(DATA_DIR, "piper", "voices")
...
MAIL_ATTACHMENTS_DIR = os.getenv("AIGENCHAIN_MAIL_ATTACHMENTS_DIR", os.path.join(DATA_DIR, "mail-attachments"))
...
def internal_api_base() -> str:
    """Base URL for in-process loopback calls to Aigenchain's own API.

    Agent tools and background jobs reach admin-gated routes by calling the
    running server over HTTP. Resolution order:
      1. AIGENCHAIN_INTERNAL_BASE  - explicit override (e.g. behind a TLS proxy).
    ...
    """
    override = os.environ.get("AIGENCHAIN_INTERNAL_BASE")
    if override:
